...
threshold is counted in what the client sends to the API — measured at 0.20×–1.92× across 22
...
when it doesn't. The run holds 22 rows — the measured servers it covered when it last ran —
and [results/leaderboard.md](results/leaderboard.md) prints a claude number for the 14 that
...
shows what you are choosing between: across the 85 servers measured, cost spans **1,700×**,
...
*(85 of 106 popular servers measured, each row dated by its own most recent sweep — full table in
