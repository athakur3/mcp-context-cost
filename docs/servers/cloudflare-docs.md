...
An Anthropic request carries 252 of those tokens as tool definitions, and Claude counts those at **756**.
...
| | tokens | |
|---|---:|---|
| o200k, full capture | 422 | the badge number — every byte `tools/list` returned |
| o200k, Anthropic fields only | 252 | 40.3% of the capture is MCP-only metadata |
| **Claude, same fields** | **756** | 1.79× the badge number |
