...
An Anthropic request carries 238 of those tokens as tool definitions, and Claude counts those at **700**.
...
| | tokens | |
|---|---:|---|
| o200k, full capture | 238 | the badge number — every byte `tools/list` returned |
| o200k, Anthropic fields only | 238 | 0.0% of the capture is MCP-only metadata |
| **Claude, same fields** | **700** | 2.94× the badge number |
